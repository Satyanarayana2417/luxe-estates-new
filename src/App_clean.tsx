...
const App = () => {
  ...
  const [showPageShine, setShowPageShine] = useState(false);

  const triggerPageShine = () => {
    // Check if user prefers reduced motion
    if (window.matchMedia('(prefers-reduced-motion: reduce)').matches) {
      return;
    }
    
    setShowPageShine(true);
  ...
  return (
    <>
      <LoadingScreen isVisible={isLoading} onLoadingComplete={triggerPageShine} />
      
      {/* Page Shine Sweep Overlay */}
      {showPageShine && (
        <div className={`page-shine-overlay ${showPageShine ? 'active' : ''}`} />
      )}
